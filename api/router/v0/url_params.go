...
func decodeBase64Param(ctx context.Context, base64Param string) (string, error) {
	if base64Param == "" {
		return "", httperror.New(ctx, 400, "the base 64 param cannot be empty")
	}
	value, err := base64.RawURLEncoding.DecodeString(base64Param)
	if err != nil {
		value, err = base64.StdEncoding.DecodeString(base64Param)
		if err != nil {
			return "", httperror.New(ctx, 400, "Cannot decode the param with base64. Original value: %s. Error: %w", base64Param, err)
		}
